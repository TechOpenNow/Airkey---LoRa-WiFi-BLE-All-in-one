...
class module:
    ...
    def __init__(self, uart_id=0, tx=0, rx=1, baudrate=115200, timeout=1):
        """
        Initializes UART communication for RP2040.

        :param uart_id: UART number (0)
        :param tx: TX pin number - 0
        :param rx: RX pin number - 1
    ...
    def open(self):
        """Initializes UART."""
        try:
            self.ser = UART(self.uart_id, baudrate=self.baudrate,
                tx=Pin(self.tx), rx=Pin(self.rx), timeout= self.timeout)
            
            print(f"UART{self.uart_id} opened at {self.baudrate} baud.")
            
        except Exception as e:
            print(f"Error opening UART: {e}")

    def send_data(self, command, wait_for_response=True, timeout=2000):
        """
        ...
        :param timeout: Delay before reading the response (default: 2 second)
        ...
        if self.ser:
            ...
            print("UART not initialized. Call open() first.")
            return None
    ...
    def receive_data(self):
        ...
        :return: Decoded response or None
        """
        if self.ser.any():
            data = self.ser.read()
            print(data)
    ...
    def receive_hex_data(self):
        ...
        """
        if self.ser.any() > 0:
            data = self.ser.read()   # Read all available bytes
